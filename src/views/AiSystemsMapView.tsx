...
const Stat = ({ label, value }: { label: string; value: React.ReactNode }) => (
  <div className="border border-border p-5">
    <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground/50 mb-2">{label}</p>
      ...
      <div className="max-w-7xl mx-auto px-6">
        <ScrollReveal>
          ...
          <p className="font-mono text-sm text-muted-foreground leading-relaxed max-w-3xl mb-8">
            ...
            data centers and capital the whole thing rests on. Trace what any company <span className="text-emerald-300">depends on</span>,
            what <span className="text-sky-300">feeds it</span>, and where the <span className="text-amber-300">chokepoints</span> are.
          </p>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-12">
            <Stat label="Entities" value={ONTOLOGY_COUNTS.nodes} />
            <Stat label="Dependencies" value={ONTOLOGY_COUNTS.edges} />
            <Stat label="Chokepoints" value={<span className="text-amber-300">{ONTOLOGY_COUNTS.chokepoints}</span>} />
